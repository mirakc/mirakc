...
pub(super) fn mount_entries<T, E, R, S, O, W>(
    config: &Config,
    router: Router<Arc<AppState<T, E, R, S, O, W>>>,
...
fn mount_file<T, E, R, S, O, W>(mount: &MountConfig) -> Router<Arc<AppState<T, E, R, S, O, W>>>
where
    T: Send + Sync + 'static,
    E: Send + Sync + 'static,
    R: Send + Sync + 'static,
